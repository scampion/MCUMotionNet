...
import json 
from fomo_trainer_grayscale import create_fomo_model, fomo_loss_function, FomoDataGenerator
...
OBJECT_WEIGHT = 100.0
MODEL_SAVE_PATH = 'person_detector_fomo_grayscale.h5' # Keras H5 format for grayscale model
# --- End Configuration ---
    ...
    for obj in data.get('objects', []):
        if obj.get('classTitle') == 'person': # Assuming PERSON_CLASS_ID corresponds to 'person'
            ...
            if points and len(points) == 2 and len(points[0]) == 2 and len(points[1]) == 2:
                ...
                # Removed verbose print from original: print(f"Loaded person annotation: ({x_center_norm}, {y_center_norm}) from {ann_file_path}")
            else:
...
if __name__ == '__main__':
    print("Loading training data for grayscale model...")
    train_image_files, train_annotations = load_dataset_split(DATA_BASE_DIR, 'train')
    
    print("Loading validation data for grayscale model...")
    ...
    train_generator = FomoDataGenerator(
        ...
        input_shape=INPUT_SHAPE, # This will be (H, W, 1)
    ...
    # Create the FOMO model for grayscale
    ...
    print(f"\nStarting training for person detection (grayscale model)...")
    ...
    try:
        ...
        print("\nTraining completed.")
        print(f"Grayscale model saved to {MODEL_SAVE_PATH} (if validation improved or no validation).")
